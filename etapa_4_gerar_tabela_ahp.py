...
def calc_f3_robustness(sol, params):
    # f3: Robustez Sistêmica (Média das Folgas)
...
def calc_f4_stability(sol, params):
    # f4: Estabilidade de Carga (Entropia de Recursos)
    cargas = np.zeros(params['m'])
    for t, ag in enumerate(sol):
        cargas[ag] += params['a'][ag, t]
    ...
    p_pos = p[p > 0]
    # Correto: Baseado em carga (p), não contagem
...
def main():
    ...
    # --- NORMALIZAÇÃO (0 a 1) para Score ---
    # f1 (Min), f2 (Min), f4 (Min): (Max - x) / Range
    # f3 (Max): (x - Min) / Range
    ranges = {}
    ...
    # Converte Scores Normalizados de f3 e f4 para escala 0-100 para exibição
